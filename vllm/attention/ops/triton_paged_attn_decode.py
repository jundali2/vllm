...
def _paged_attn_decode_v1_w_dot_kernel(
    ...
    if alibi_slopes is None:
        ...
        alibi_slope = tl.load(alibi_slopes + kv_head_idx*QUERY_GRP_SZ + q_grp_offs, mask=q_grp_offs < QUERY_GRP_SZ, other=0.0)
    ...
    for b in range(num_kv_blks):
        ...
        # v: [KV_BLK_SZ, HEAD_SZ]
        v_0 = tl.load(v_cache_ptr + kv_blk_offs, mask=kv_mask, other=0.0).to(dtype)
        if v_0.dtype.is_fp8():
            v = v_0.to(tl.float32) * v_scale
        else:
            v = v_0
...
def _paged_attn_decode_v2_w_dot_kernel(
    ...
    #This conversion is needed. Otherwise, this causes a compilation error with mismatch types in the loop below
    #num_kv_blks = tl.cdiv(seq_end_idx - seq_start_idx, KV_BLK_SZ).to(tl.int32)
    num_kv_blks = tl.cdiv(seq_end_idx - seq_start_idx, KV_BLK_SZ)
